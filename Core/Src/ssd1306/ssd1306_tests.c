...
extern uint16_t _ADCvol;
extern uint16_t _ADCcur;
extern uint16_t _cam_model;
extern uint16_t _cam_version;
extern bool _status_power;

char OLED_Text[100] = {0, };
...
void ssd1306_camStatus() {
    //ssd1306_Fill(Black);
	ssd1306_FillRectangle(1, 1, SSD1306_WIDTH, 23, White);
	ssd1306_FillRectangle(1, 24, SSD1306_WIDTH, SSD1306_HEIGHT, Black);
    if(_status_power == TRUE)
    {
    	uint8_t v03 = _ADCvol/100;
    	uint8_t v12 = _ADCvol - (v03*100);

    	ssd1306_SetCursor(4, 4);
        ssd1306_WriteString("Camera ON", Font_11x18, Black);
        //ssd1306_SetCursor(4, 24);
        ssd1306_SetCursor(4, 26);

        sprintf(OLED_Text, "%d.%02d V", v03, v12);
        ssd1306_WriteString(OLED_Text, Font_11x18, White);
        ssd1306_SetCursor(4, 44);
        sprintf(OLED_Text, "%d mA", _ADCcur);
        ssd1306_WriteString(OLED_Text, Font_11x18, White);
    }
    else
	{
    	ssd1306_SetCursor(4, 4);
    	ssd1306_WriteString("Maestro-2", Font_11x18, Black);

	}
    ssd1306_UpdateScreen();
}

void ssd1306_camInfo() {
    //ssd1306_Fill(Black);
	ssd1306_FillRectangle(1, 1, SSD1306_WIDTH, 23, White);
	ssd1306_FillRectangle(1, 24, SSD1306_WIDTH, SSD1306_HEIGHT, Black);
    if(_status_power == TRUE)
    {
    	uint8_t ver1 = (uint8_t)(_cam_version >> 8);
    	uint8_t ver2 = (uint8_t)(_cam_version & 0x00FF);

    	ssd1306_SetCursor(4, 4);
        ssd1306_WriteString("CAM Info", Font_11x18, Black);
        //ssd1306_SetCursor(4, 24);
        ssd1306_SetCursor(4, 30);

        sprintf(OLED_Text, "Model %06x", _cam_model);
        ssd1306_WriteString(OLED_Text, Font_7x10, White);
        ssd1306_SetCursor(4, 44);
        sprintf(OLED_Text, "Ver %d.%d", ver1, ver2);
        ssd1306_WriteString(OLED_Text, Font_7x10, White);
    }
    /*else
	{
...
void ssd1306_Display(OLEDmode mode)
{
	...
	case CAM_RX_FAIL:
    	ssd1306_SetCursor(4, 4);
        ssd1306_WriteString("Unidenti", Font_11x18, Black);
		break;
	default:
	}
	ssd1306_UpdateScreen();
