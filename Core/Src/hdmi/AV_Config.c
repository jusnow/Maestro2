...
#if defined(__MDIN_i550_USB_V100__) || defined(_WONWOO)
#define DEV_EMBEDDED		// for aux sync embedded output
#endif

//#define USE_YC_MUX_INPUT	// for mux656 test
...
#ifdef USE_YC_MUX_INPUT
	TclkSel = 0;	// ratio of output tmds clk to input pclk,  0(x0.5), 1(x1 default), 2(x2), 3(x4)
#endif
...
//	B_Data[0] = (VModesTable[ModeTblIndex].PixRep) & LOW_BYTE;		// Set pixel replication field of 0x08
	B_Data[0] = 0x00;		// Set pixel replication field of 0x08 - fix no repeat !!!
...
#ifdef USE_YC_MUX_INPUT
	B_Data[2] = BITS_IN_DEMUX_EN;	// input de-mux enable
#else
	B_Data[2] = BITS_IN_DEMUX_DIS;	// input de-mux disable
#endif
...
#ifdef _DEF_HDMI_OUTPUT_CSC
	SIL9136_SetCSC(g_ucHDMIoutCSC);
#else	// _DEF_HDMI_OUTPUT_CSC
	SIL9136_SetCSC(Sil9136CSC);
#endif	// _DEF_HDMI_OUTPUT_CSC
...
{
	#if (_LENS_18X_4K2_ZM == _LENS_TYPE)
	if (!IsHDMI_Sink())		// if (IsHDMI_Sink())
	#else	// (_LENS_18X_4K2_ZM == _LENS_TYPE)
	if (!IsHDMI_Sink())
	#endif	// (_LENS_18X_4K2_ZM == _LENS_TYPE)
	{
	...
	ReadModifyWriteTPI(TPI_SYNC_GEN_CTRL, BIT_5, Data[2]);	// [5] YC Mux Mode
